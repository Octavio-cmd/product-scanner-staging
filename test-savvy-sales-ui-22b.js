...
const block = sku => { const h = slot(); const a = h.indexOf('>' + sku + '</span>'); if (a < 0) return ''; const b = h.indexOf('border-top:1px solid var(--bd)', a); return h.slice(a, b > 0 ? b : undefined); };
...
section('6–11 — loading / confirmed zero / failure / unconfirmed / incomplete / malformed');
{
  ...
  const b = block(S1PK);
  checkTrue('6 loading shows "⏳ Consultando ventas reales..." and no number', b.includes('⏳ Consultando ventas reales...') && !/gross packs|\b0\b/.test(b.replace(S1PK, '')), b);
  h.release(S1PK, { body: LEGO_1PK() }); await settleSales();
  checkTrue('6b after the response the numbers appear', block(S1PK).includes('<strong>155</strong> gross packs sold'), block(S1PK));
}
await scanWith(LEG, 'LEGO', [sbProd(S1PK, 3)], [], { [S1PK]: { mode: 'zero' } }, { inv: LEGO_INV });
checkTrue('7 confirmed zero shows a real 0 in every window', ['7d', '30d', '90d'].every(k => block(S1PK).includes(k + ': <strong>0</strong> gross packs sold · 0 orders · 0.00 packs/day')), block(S1PK));
const failCases = {
...
for (const [name, spec] of Object.entries(failCases)) {
  ...
  const b = block(S1PK);
  checkTrue(name + ' → "⚠️ Ventas no confirmadas", no zero, no numbers', b.includes('⚠️ Ventas no confirmadas') && !b.includes('gross packs') && !/<strong>\d+<\/strong>/.test(b), b);
}
...
check('LEGO 1pk 7d', b1.includes('7d: <strong>155</strong> gross packs sold · 48 orders · 22.14 packs/day'), true);
check('LEGO 1pk 30d', b1.includes('30d: <strong>234</strong> gross packs sold · 105 orders · 7.80 packs/day'), true);
check('LEGO 1pk 90d', b1.includes('90d: <strong>273</strong> gross packs sold · 136 orders · 3.03 packs/day'), true);
check('LEGO 2pk 7d', b2.includes('7d: <strong>15</strong> gross packs sold · 5 orders · 2.14 packs/day'), true);
check('LEGO 2pk 30d', b2.includes('30d: <strong>32</strong> gross packs sold · 7 orders · 1.07 packs/day'), true);
check('LEGO 2pk 90d', b2.includes('90d: <strong>38</strong> gross packs sold · 9 orders · 0.42 packs/day'), true);
checkTrue('13/14/15 7d, 30d, 90d rendered in order', b1.indexOf('7d:') < b1.indexOf('30d:') && b1.indexOf('30d:') < b1.indexOf('90d:'), b1);
checkTrue('16 gross label present + nested-window note', slot().includes('gross packs sold') && slot().includes('Gross packs sold: incluye órdenes con reembolso') && slot().includes('7d ⊂ 30d ⊂ 90d') && slot().includes('no se suman'), slot());
...
check('20 2pk physical units distinguished from packs', ['7d', '30d', '90d'].map(k => b2.includes(k + ': <strong>' + ({ '7d': 15, '30d': 32, '90d': 38 })[k] + '</strong> gross packs sold') && b2.includes('(= ' + ({ '7d': 30, '30d': 64, '90d': 76 })[k] + ' physical units)')), [true, true, true]);
checkTrue('20b 1pk shows no redundant physical-units text', !b1.includes('physical units'), b1);
checkTrue('21 refund warnings rendered (30d: 1, 90d: 3; 7d none)', b1.includes('⚠️ 1 refunded order included in gross sales') && b1.includes('⚠️ 3 refunded orders included in gross sales') && (b1.match(/refunded order/g) || []).length === 2, b1);
checkTrue('22 refunds not subtracted (30d stays 234, 90d stays 273)', b1.includes('<strong>234</strong>') && b1.includes('<strong>273</strong>') && !b1.includes('<strong>233</strong>') && !b1.includes('<strong>270</strong>'), b1);
checkTrue('23 cancellations shown as excluded, not added back', b1.includes('Excluded: 1 cancelled order / 1 pack') && b1.includes('Excluded: 2 cancelled orders / 3 packs') && b1.includes('Excluded: 3 cancelled orders / 5 packs') && !b1.includes('<strong>156</strong>'), b1);
...
await legoScan(LEGO_SALES());
{ const h = slot(); checkTrue('blocks ordered 1pk (-1, -1pk) then 2pk', h.indexOf('>' + S1 + '</span>') < h.indexOf('>' + S2PK + '</span>') && h.indexOf('>' + S1PK + '</span>') < h.indexOf('>' + S2PK + '</span>'), h); }
...
}
{
  ...
  await settleSales();
  checkTrue('27 older same-UPC response ignored (current values kept)', block(S1PK).includes('<strong>155</strong>') && !slot().includes('999'), slot());
}

{
  await legoScan(LEGO_SALES());
  checkTrue('26c precondition: LEGO sales painted', block(S1PK).includes('<strong>155</strong>'), slot());
...
['renderSplitCalculatorHTML', 'getDemandTier', 'computeSplit', 'updateSplitCalc', 'addSplitPacksToCSV', 'exportCSV', 'applyVerdict', 'callClaude', 'finishAnalyze']
